...
from mode_matrixfactorization import *
...
def make_datasets(train_path="data_train.csv",predict_path="data_test.csv"):
    ...
    X_train, X_test, y_train, y_test = train_test_split(df_train[['User','Movie']], df_train['Rating'], test_size=0.33, random_state=89)
...
def get_predicts(data_train,data_test,spark_context):
    ...
    ratings_train = load_data_as_scipy(train_file)
    ratings_test = load_data_as_scipy(test_file)

    pred_sgd = implementation_SGD(ratings_train ,ratings_test)
    pred_als = implementation_ALS(ratings_train,ratings_test)

    implementation_global_mean(ratings_train, ratings_test)
    implementation_user_mean(ratings_train, ratings_test)
    implementation_item_mean(ratings_train, ratings_test)
...
def get_weights(all_preds,target):
    ...
    weights = dict(zip(["baseline_pred","slopeone_pred","knn_ub_pred","knn_ib_pred","svd_pred","svdpp_pred","als_pred","pyfm_pred"], linreg.coef_))
